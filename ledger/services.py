...
def calc_summary(transactions):
    ...
    total_income = transactions[transactions['type'] == '수입']['amount'].sum()
    
    # type이 '지출'인 행만 골라서 amount의 합계를 구함
    total_expense = transactions[transactions['type'] == '지출']['amount'].sum()
...
def get_category_stats(transactions):
    ...
    # 1. '지출' 데이터만 필터링
    expense_df = transactions[transactions['type'] == '지출']
